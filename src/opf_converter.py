...
def filter_property_tokens(properties):
    tokens = []
...
def encode_local_href(href):
    ...
    return quote(unquote(base).replace("\\", "/").replace(" ", "_"), safe="/%:@!$&'()*+,;=-._~") + frag
...
class Opf_Converter(object):
    ...
    def __init__(self, opf2data, spine_properties, manifest_properties, mo_properties, man_ids):
        ...
        self.uniqueid = None
        self.uid = ""
        self.first_identifier_id = None
        self.first_identifier_content = None
        self.title_cnt = 0
        self.creator_cnt = 0
            ...
            if tname == "itemref" and prefix.endswith("spine"):
                ...
                if idref in self.sprops:
                    if props != "":
                        ...
                        props = self.sprops[idref]
                props = filter_property_tokens(props)
                if props != "":
                    tattr["properties"] = props
                elif "properties" in tattr:
                    del tattr["properties"]
                res.append(taginfo_toxml((tname, tattr, None)))
                continue
    ...
    def map_meta(self, tname, tattr, tcontent):
        ...
        if aname and "property" not in tattr:
            cleaned = {"name": aname}
            cleaned["content"] = acont
            tattr = cleaned
    ...
    def map_dc(self, tname, tattr, tcontent):
        ...
        if tattr is None:
            tattr = {}
        tattr = self.clean_namespace_attrs(tattr)
        ...
        if tname in  ["dc:creator", "dc:contributor"]:
            ...
            role = None
            fileas = None
            for key in list(tattr.keys()):
                if key == "opf:role" or key.endswith(":role"):
                    role = tattr[key]
                    del tattr[key]
                elif key == "opf:file-as" or key.endswith(":file-as"):
                    fileas = tattr[key]
                    del tattr[key]
            outtags.append([tname, tattr, tcontent])
            if role is not None:
        ...
        if tname == "dc:identifier":
            if "id" in tattr:
                ...
                if self.first_identifier_id is None:
                    self.first_identifier_id = tattr["id"]
                    self.first_identifier_content = tcontent
            for key in list(tattr.keys()):
                if key == "opf:scheme" or key.endswith(":scheme"):
                    scheme = tattr[key].lower()
                    del tattr[key]
                    if scheme == "uuid" and not is_uuid_value(tcontent):
                        break
                    if not tcontent.startswith("urn:") :
                        tcontent = "urn:" + scheme + ":" + tcontent
                    break
            if tattr.get("id", "") == self.uniqueid:
                self.uid = tcontent
            if tattr.get("id", "") == self.first_identifier_id:
                self.first_identifier_content = tcontent
            outtags.append([tname, tattr, tcontent])
            return outtags
    ...
    def clean_namespace_attrs(self, tattr):
        cleaned = {}
        for key, value in tattr.items():
    ...
    def get_opf3(self):
        ...
        if self.uid == "" and self.first_identifier_id is not None:
            opfdata = re.sub(
                r'unique-identifier="[^"]*"',
                'unique-identifier="%s"' % self.first_identifier_id,
                opfdata,
                count=1,
            )
    ...
    def get_uid(self):
        if self.uid:
            return self.uid
        return self.first_identifier_content or ""
